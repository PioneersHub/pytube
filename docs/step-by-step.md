...
pretalx:
  event_slug: "pyconde-pydata-2026"  # Your event slug
  questions_map:
...
- Create JSON files in `projects/pyconde-pydata-2026/records/`

AI descriptions are generated by a separate command (`fetch` no longer does this):

```bash
# Generate AI teaser/description texts for the fetched records
pytube records generate-descriptions

# Re-generate and overwrite existing descriptions
...
**Verify**: Check that files exist in `projects/pyconde-pydata-2026/records/`
```bash
ls -la projects/pyconde-pydata-2026/records/ | head -10
...
cat projects/pyconde-pydata-2026/records/ABC123.json | jq '.sm_teaser_text, .sm_short_text'
```
...
```bash
mkdir -p projects/pyconde-pydata-2026/videos/downloads
find "<SOURCE>" -type f \( -iname '*.mp4' -o -iname '*.mov' -o -iname '*.mkv' \) \
  -exec ln -sf {} "$PWD/projects/pyconde-pydata-2026/videos/downloads/" \;
```
...
# Update video metadata with descriptions (default template: youtube_2026.txt)
...
pytube youtube schedule --start "2026-05-01T10:00:00" --interval 6h --preview
```
...
echo "Records: $(ls projects/pyconde-pydata-2026/records/ | wc -l)"
echo "Updated: $(ls projects/pyconde-pydata-2026/videos/youtube/video_records_updated/ | wc -l)"
echo "Published: $(ls projects/pyconde-pydata-2026/videos/youtube/video_published/ | wc -l)"

# Check for pending notifications
ls -la projects/pyconde-pydata-2026/speaker_to_email/
ls -la projects/pyconde-pydata-2026/linked_in_to_post/
```
...
mv projects/pyconde-pydata-2026/videos/youtube/video_published/ABC123.json projects/pyconde-pydata-2026/videos/youtube/video_records_updated/
pytube notify check --auto-post

# Skip problematic video
mv projects/pyconde-pydata-2026/videos/youtube/video_records_updated/ABC123.json projects/pyconde-pydata-2026/videos/youtube/video_published/

# Regenerate descriptions
rm projects/pyconde-pydata-2026/records/ABC123.json
pytube records fetch
pytube records generate-descriptions --replace
...
- `projects/pyconde-pydata-2026/records/*.json` - Session data
- `projects/pyconde-pydata-2026/videos/youtube_pycon.json` - Video mappings
