...
def sample_session_data():
    ...
    return {
        "UXTCZC": {
            ...
            "track": {"en": None, "de": None, "id": 5205, "name": {"en": "PyCon: MLOps & DevOps"}},
        },
        "ABCD12": {
            "code": "ABCD12",
            ...
            "track": {
                ...
                "de": None,
                "id": 5215,
                "name": {"en": "PyData: PyData & Scientific Libraries Stack"},
            },
        },
        "WXYZ34": {"code": "WXYZ34", "title": "No Track Session", "track": None},
    }
...
class TestSplitPyconPydata:
    ...
    def test_track_pattern_matching_new_structure(self, mock_config):
        ...
        with patch("manager.scripts.video_organizer.conf", mock_config):
            video = {"code": "TEST1", "track": {"name": {"en": "PyCon: MLOps & DevOps"}}}
            result = video_organizer.split_pycon_pydata(video)
    ...
    def test_track_pattern_case_insensitive(self, mock_config):
        ...
        with patch("manager.scripts.video_organizer.conf", mock_config):
            video = {"code": "TEST2", "track": {"name": {"en": "PyData: Scientific Computing"}}}
            result = video_organizer.split_pycon_pydata(video)
    ...
    def test_unmatched_track(self, mock_config):
        ...
        with patch("manager.scripts.video_organizer.conf", mock_config):
            video = {"code": "TEST4", "track": {"name": {"en": "Unknown Track"}}}
            result = video_organizer.split_pycon_pydata(video)
            assert result is None
...
class TestAssignVideoToChannel:
    ...
    def test_assign_videos(self, mock_config, sample_session_data, tmp_path):
        ...
        with (
            patch("manager.scripts.video_organizer.conf", mock_config),
