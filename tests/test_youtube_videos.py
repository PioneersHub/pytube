...
def test_list_all_videos(mock_build, mock_get_authenticated_service):
    ...
    mock_request.execute.return_value = {"items": [{"id": "video1"}, {"id": "video2"}]}
...
def test_publish_dates_generator_with_delta():
    ...
    assert dates == [datetime(2023, 1, 1), datetime(2023, 1, 2), datetime(2023, 1, 3)]
...
def test_publish_dates_generator_raises_value_error_without_end_or_delta():
    start = datetime(2023, 1, 1)
    with pytest.raises(
        ValueError, match=r"Either end \(datetime\) or delta \(release every timeperiod\) must be provided"
    ):
