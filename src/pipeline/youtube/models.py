...
class YouTubeStatus(BaseModel):
    ...
    privacyStatus: Literal["private", "unlisted", "public"] = Field(
        "unlisted",
    ...
    publishAt: Optional[datetime] = Field(None, description="Scheduled publish time (ISO 8601)")
...
class YouTubeMapping(BaseModel):
    ...
    mappings: dict[str, str] = Field(
        ...
        description="Dictionary mapping pretalx_id to youtube_id"
    )
    total_count: int = 0
    created_at: datetime = Field(default_factory=datetime.utcnow)
    source: str = Field("manual", description="How the mapping was created")

    def get_youtube_id(self, pretalx_id: str) -> Optional[str]:
