...
class YouTubeMetadataConfig(BaseModel):
    """YouTube-specific metadata configuration."""

    category_id: int = Field(default=28, description="Category ID (28 = Science & Technology)")
    default_language: str = Field(default="en", description="Default language")
    privacy_status: Literal["private", "unlisted", "public"] = Field(default="unlisted")
    license: Literal["youtube", "creativeCommon"] = Field(default="youtube")
    embeddable: bool = Field(default=True)
    tags: list[str] = Field(
        default_factory=lambda: ["Python", "PyConDE", "PyData", "Programming", "Software Development", "Open Source"],
...
class PreparedVideoMetadata(BaseModel):
    ...
    youtube_metadata: YouTubeMetadataConfig = Field(
        default_factory=YouTubeMetadataConfig, description="YouTube-specific settings"
    )
